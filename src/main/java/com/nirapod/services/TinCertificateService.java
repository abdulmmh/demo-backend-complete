...
public class TinCertificateService {
    ...
    public void generateCertificate(Tin tin, Taxpayer taxpayer, HttpServletResponse response) throws Exception {
        ...
        // Watermark
        try {
            Image watermark = Image.getInstance(new ClassPathResource("images/watermark.png").getURL());
        ...
        // Logo
        try {
            Image logo = Image.getInstance(new ClassPathResource("images/govt_logo.png").getURL());
        ...
        // Header
        Paragraph p1 = new Paragraph("Government of the People's Republic of Bangladesh", titleFont);
        ...
        Paragraph p3 = new Paragraph("Taxpayer's Identification Number (TIN) Certificate", subTitleFont);
        ...
        // Name logic
        String name = taxpayer.getTaxpayerType().getTypeName().equalsIgnoreCase("Company")
                ? taxpayer.getCompanyName()
                : taxpayer.getFullName();

        // Certification
        Paragraph certifyText = new Paragraph();
        certifyText.add(new Chunk("This is to Certify that ", normalFont));
        certifyText.add(new Chunk(name, boldFont));
        certifyText.add(new Chunk(" is a Registered Taxpayer of National Board of Revenue under the jurisdiction of ", normalFont));
        certifyText.add(new Chunk("Taxes Circle-" + tin.getTaxCircle() + ", Taxes Zone-" + tin.getTaxZone() + ".", boldFont));
        certifyText.setAlignment(Element.ALIGN_JUSTIFIED);
        ...
        document.add(new Paragraph("6) Status : " + taxpayer.getTaxpayerType().getTypeName(), boldFont));
        ...
        // ✅ QR Generate FIRST
        BufferedImage qrBuffered = generateQR(
                "TIN: " + tin.getTinNumber() + "\nName: " + name
        );
